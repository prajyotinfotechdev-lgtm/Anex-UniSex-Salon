...
const MOCK_SERVICES = [
  { id: 'srv_1', name: 'Signature Haircut', duration: 45, price: 800, category: 'Hair' },
  { id: 'srv_2', name: 'Beard Sculpting', duration: 30, price: 400, category: 'Grooming' },
  { id: 'srv_3', name: 'Premium Balayage', duration: 120, price: 4500, category: 'Color' },
  { id: 'srv_4', name: 'Deep Tissue Massage', duration: 60, price: 1500, category: 'Spa' },
];
...
export function ServiceSelector() {
  ...
  const filteredServices = MOCK_SERVICES.filter(s => s.name.toLowerCase().includes(search.toLowerCase()));
  const hasSelection = state.serviceIds.length > 0;
  const totalPrice = state.serviceIds.reduce((acc, id) => {
    const s = MOCK_SERVICES.find(srv => srv.id === id);
